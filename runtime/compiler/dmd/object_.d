...
private
{
    static import cImports = rt.cImports;/+: cImports.memcmp,cImports.memcpy,cImports.memmove,cImports.realloc,cImports.free,cImports.onOutOfMemoryError;+/
...
class TypeInfo_StaticArray : TypeInfo
{
    ...
    override void swap(void* p1, void* p2)
    {
        ...
        {   size_t o = u * sz;
            cImports.memcpy(tmp, p1 + o, sz);
            cImports.memcpy(p1 + o, p2 + o, sz);
            cImports.memcpy(p2 + o, tmp, sz);
        }
        if (pbuffer)
            delete pbuffer;
    }
...
class TypeInfo_Delegate : TypeInfo
{
    ...
    {
        alias int delegate() dg;
        return cImports.memcmp(p1,p2,dg.sizeof);
    }
...
class TypeInfo_Struct : TypeInfo
{
    ...
    override equals_t equals(in void* p1, in void* p2)
    {
        if (p1 == p2)
            ...
            // BUG: relies on the GC not moving objects
            return cImports.memcmp(p1, p2, init.length) == 0;
    }
        ...
        if (p1 != p2)
        {
            if (p1)
            {
                if (!p2)
                    ...
                    // BUG: relies on the GC not moving objects
                    return cImports.memcmp(p1, p2, init.length);
            }
            else
                return -1;
        }
...
extern (C) void rt_attachDisposeEvent(Object h, DEvent e)
{
    synchronized (h)
    {
        ...
        auto p = cImports.realloc(m.devt.ptr, DEvent.sizeof * len);
        if (!p)
            cImports.onOutOfMemoryError();
