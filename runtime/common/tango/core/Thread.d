...
version( Win32 )
{
    private
    {
        ...
        extern (Windows) uint thread_entryPoint( void* arg )
        {
            ...
            {
                version(none){ // dmd 1.046 seems to have problems with this
                    if (obj.m_isDaemon){
                        printf("unhandled exception in daemon thread\n");
                        auto e=cast(Exception)o;
                        if (e !is null){
                            e.writeOut(delegate void(char[]s){ printf("%.*s",s.length,s.ptr); });
                        } else if (o !is null) {
                            auto s=o.toString;
                            printf("%.*s\n",s.length,s.ptr);
                        }
...
else version( Posix )
{
    private
    {
        ...
        extern (C) void* thread_entryPoint( void* arg )
        {
            ...
            {
                version(none){ // dmd 1.046 seems to have problems with this version
                    if (obj.m_isDaemon){
                        printf("unhandled exception in daemon thread\n");
                        auto e=cast(Exception)o;
                        if (e !is null){
