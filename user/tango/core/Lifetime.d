...
class OnFirstDestructor{
    ...
    this(void delegate() action,Object[] objs=null){
        hiddenAddrs=(cast(size_t*)malloc(objs.length*size_t.sizeof))[0..objs.length];
        foreach (i,obj;objs){
    ...
    void nullify(){
        synchronized (this){ // should use a spinlock, as the monitor might have been collected...
            if (!triggered) {
                triggered=true;
    ...
    void addObject(Object obj){
        if (obj !is null){
            synchronized(this){
                auto newP=cast(size_t*)realloc(hiddenAddrs.ptr,(hiddenAddrs.length+1)*size_t.sizeof);
                hiddenAddrs=newP[0..hiddenAddrs.length+1];
                hiddenAddrs[hiddenAddrs.length-1] = ~(cast(size_t)(cast(void*)obj));
                rt_attachDisposeEvent(obj,&_destructorCallback);
            }
        }
    }
